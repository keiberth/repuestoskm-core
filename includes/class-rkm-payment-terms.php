...
class RKM_Payment_Terms {
        ...
        return [
            ...
            'terms' => [
                'cash' => [
                    ...
                    'label'        => 'Contado',
                    'active'       => 1,
                    'instructions' => 'Pago realizado por fuera de la plataforma y validado por administracion.',
                ],
                'credit' => [
                    'key'          => 'credit',
                    'label'        => 'Credito',
    ...
    public function handle_submission() {
        if (!$this->is_payment_terms_submission()) {
            return;
        }

        if (!is_user_logged_in() || !self::can_access()) {
            $this->set_flash_notice('error', 'No tenes permiso para gestionar condiciones de pago.');
            $this->redirect_back();
        }
    ...
    private function is_payment_terms_submission() {
        if (!$this->is_post_request()) {
            return false;
        }

        if (!isset($_POST['rkm_payment_terms_nonce'])) {
            return false;
        }
